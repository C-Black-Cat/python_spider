...
# 上面装饰器的使用等价于
# wrapper_obj = debug(say)
# wrapper_obj("xxx")
```

![image-20230715173503909](D:\Typora\my_file\图片\image-20230715173503909.png)

![image-20230715174441475](D:\Typora\my_file\图片\image-20230715174441475.png)
...
> 其实这里 `@logging(level='DEBUG')` 等价于 `@wrapper`



### 基于类的装饰器

装饰器函数其实是这样一个接口约束，它必须接受一个callable对象作为参数，然后返回一个callable对象。在Python中一般 `callable` 对象都是函数，但也有例外。只要某个对象重载了`__call__()`方法，那么这个对象就是callable的。
...
# 类装饰器等价于
# log_obj = Logging(level="INFO")
# wrapper_obj = log_obj(say)
# wrapper_obj("xxx")
```
...
Python中的**抽象基类**（`Abstract Base Classes`，简称 `abc`）是一种特殊的类，它用于定义一组抽象方法，这些方法必须在子类中被实现。**抽象基类本身不能被实例化，而是用于定义子类所需实现的接口**。在定义上来说，跟java里面的interface有异曲同工之妙。

在接触抽象基类之前，我们先看一下，python里面可以怎样控制子类继承某个类之后，要强制重写父类的方法，如果不重写的话，就抛出异常：

**重要：子类不实现抽象基类方法会报错的原理**
...
# 注意Animal这个基类一定要继承ABC类，否则即使使用了装饰器@abstractmethod，子类不实现父类的方法也不会报错
class Animal(ABC):
    @abstractmethod
    def say(self):
        pass
...
![image-20230715181344698](D:\Typora\my_file\图片\image-20230715181344698.png)

### 抽象基类不可实例化

![image-20230715181727814](D:\Typora\my_file\图片\image-20230715181727814.png)
...
![image-20230715182132090](D:\Typora\my_file\图片\image-20230715182132090.png)
...
**property源码，property本质是一个类**

![image-20230715185040563](D:\Typora\my_file\图片\image-20230715185040563.png)

![image-20230715185114506](D:\Typora\my_file\图片\image-20230715185114506.png)

![image-20230715185134577](D:\Typora\my_file\图片\image-20230715185134577.png)
